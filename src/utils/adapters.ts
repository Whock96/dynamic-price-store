...
export const supabaseOrderToAppOrder = (
  ...
  const totalWeight = processedItems.reduce((sum, item) => sum + (item.totalWeight || 0), 0);
  const totalCubicVolume = processedItems.reduce((sum, item) => sum + (item.totalCubicVolume || 0), 0);
