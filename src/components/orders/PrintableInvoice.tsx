...
const PrintableInvoice: React.FC<PrintableInvoiceProps> = ({ order, onPrint }) => {
  ...
  const calculateTotalWithInvoice = (
    ...
    return (subtotal * (percentage / 100)) + taxSubstitutionTotal + ipiTotal + deliveryFee;
  };
  ...
  const taxSubstitutionTotal = order.items.reduce((sum, item) => sum + (item.taxSubstitutionValue || 0), 0);
  const ipiTotal = order.ipiValue || 0;
        ...
        <table className="w-full border-collapse text-xs">
          ...
          <tbody>
            {order.items.map((item, index) => {
              ...
              const priceWithInvoice = calculatePriceWithInvoice(finalPrice, halfInvoicePercentage);
              const priceWithoutInvoice = calculatePriceWithoutInvoice(finalPrice, halfInvoicePercentage);
              
              const quantityWithInvoice = calculateQuantityWithInvoice(totalUnits, halfInvoicePercentage);
              const quantityWithoutInvoice = calculateQuantityWithoutInvoice(totalUnits, halfInvoicePercentage);
              ...
              return (
                <tr key={item?.id || `item-${index}`}>
                  ...
                  <td className="border p-1 text-right">{formatCurrency(finalPrice)}</td>
                  
                  {!order.fullInvoice && halfInvoiceType === 'price' && (
                    <>
                      <td className="border p-1 text-right">{formatCurrency(priceWithInvoice)}</td>
                      <td className="border p-1 text-right">{formatCurrency(priceWithoutInvoice)}</td>
                    </>
                  )}
                  ...
                  {!order.fullInvoice && halfInvoiceType === 'quantity' && (
                    <>
                      <td className="border p-1 text-center">{quantityWithInvoice}</td>
                      <td className="border p-1 text-center">{quantityWithoutInvoice}</td>
                    </>
                  )}
          ...
          <table className="w-full">
            <tbody>
              ...
              <tr>
                ...
                <td className="py-0.5 text-right font-medium">{formatCurrency(subtotalAfterDiscount)}</td>
              </tr>
              {order.taxSubstitution && (
                <tr>
                  ...
                  <td className="py-0.5 text-right text-orange-600 font-medium">
                    +{formatCurrency(taxSubstitutionTotal)}
                  </td>
                </tr>
              )}
              {order.withIPI && (
                <tr>
                  ...
                  <td className="py-0.5 text-right text-blue-600 font-medium">+{formatCurrency(ipiTotal)}</td>
                </tr>
              )}
              {order.deliveryFee && order.deliveryFee > 0 && (
                <tr>
                  <td className="py-0.5">Taxa de Entrega:</td>
                  <td className="py-0.5 text-right font-medium">{formatCurrency(deliveryFee)}</td>
                </tr>
              )}
              ...
              {!order.fullInvoice && (
                <>
                  <tr>
                    <td className="py-0.5 font-medium">Total c/ Nota:</td>
                    <td className="py-0.5 text-right font-medium text-green-600">{formatCurrency(totalWithInvoice)}</td>
                  </tr>
                  <tr>
                    <td className="py-0.5 font-medium">Total s/ Nota:</td>
                    <td className="py-0.5 text-right font-medium text-amber-600">{formatCurrency(totalWithoutInvoice)}</td>
                  </tr>
                </>
              )}
            </tbody>
